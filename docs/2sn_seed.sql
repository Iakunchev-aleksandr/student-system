...
-- 7) Конкретные уроки на ВЕСЬ учебный год (апр–дек), паттерн выбирается по семестру
--    Диапазон, праздники и летние каникулы — РЕДАКТИРУЕМЫЕ.
--    前期 действует до 27.09, с 28.09 — 後期 (переключение автоматически по дате).
...
FROM generate_series(DATE '2026-04-06', DATE '2026-12-23', INTERVAL '1 day') AS d
JOIN (VALUES
    -- 前期 (до 27.09)
